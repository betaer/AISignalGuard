...
AI Signal Guard 先让你选择目标身份，再运行检测，并用画像专属权重生成 Identity Match Score、证据覆盖率、自然语言总结以及“为什么像 / 为什么不像”。同一组环境信号在美国消费者、内容创作者、跨境卖家和 AI 开发者画像下会得到不同解释。
...
| 模块 | 检查内容 | 用途 |
|---|---|---|
| 目标身份选择 | 美国普通用户、TikTok 创作者、跨境卖家、AI Worker 及通用分析 | 明确本次分析希望比较的数字画像 |
...
| 网络连通 | AI、创作者、商业、开发者生态、全球与中国站点的浏览器可达性 | 区分本地网络问题、浏览器限制和目标路径差异 |
...
| 画像 | 主要分析信号 |
|---|---|
| 🇺🇸 美国普通用户 | 地理、住宅/数据中心网络特征、语言、时区、DNS、WebRTC、Google、YouTube、Netflix、ChatGPT |
| 🎬 TikTok 创作者 | 网络与信誉、TikTok、Instagram、YouTube、广告平台可达性、地理一致性 |
| 🛒 跨境卖家 | IP 信誉、Shopify、Amazon、PayPal、Stripe、浏览器与位置一致性 |
| 🤖 AI Worker | OpenAI、Claude、Gemini、Cursor、Perplexity、GitHub、npm、PyPI 与网络路径 |
...
页面首次打开不会自动运行检测。只有选择画像或点击通用分析后，才会访问 IP 情报、DNS、WebRTC 和服务探针。

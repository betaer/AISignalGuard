...
| 网络连通 | AI 服务、内容平台、商业服务、开发者生态、全球与中国站点的浏览器可达性 | 收到目标端点或其备用端点响应时统一显示“可达”；无法取得响应才显示“未确认”。“可达”只代表请求路径建立，不代表登录、解锁、账号或支付功能可用 |
...
- 服务探针统一使用“可达 / 服务响应异常 / 未确认”三种可见状态。跨域端点即使不向浏览器公开 HTTP 状态，只要收到响应仍显示“可达”，并在内部保留“HTTP 状态不可读”的证据；扩展拦截、超时、DNS / TLS 失败等无法取得响应的情况才显示“⁉️ 未确认”。npm Registry 优先读取其用于健康检查的 `tiny-tarball` 包元数据，并以 `/-/ping` 作为只读备用端点，避免把 CORS 限制误报为 Registry 不可达。
